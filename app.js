...
(function() {
  ...
  function suggestedStatus() {
    ...
    if (/^(pending closure confirmation|pending recovery|action plan shared|pending dcr)$/i.test(s))
      return 'Waiting for Customer Confirmation';
    if (/^(pending pg|pending bug fix|pending cx to share information requested by pg|action plan provided by pg)$/i.test(s))
